/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_print_x.c                                       :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: jison <jison@student.42.fr>                +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/23 13:30:55 by jison             #+#    #+#             */
/*   Updated: 2022/11/24 14:05:30 by jison            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

int	ft_print_x_hex(unsigned int n, int precision, int width, t_spec *spec)
{
	int	printed;

	printed = 0;
	if (n / 16 != 0)
		printed += ft_print_x_hex(n / 16, precision - 1, width - 1, spec);
	else
		printed += ft_print_precision(precision - 1, width - 1, spec);
	printed += write(1, &HEX_LOWER[ft_abs(n % 16)], 1);
	return (printed);
}

void	config_x_spec(unsigned int arg, t_spec *spec)
{
	if (spec->alter_form && arg != 0)
		spec->head = "0x";
	if (spec->zero_pad && !spec->precision_flag)
	{
		spec->pad_chr = '0';
		if (spec->head)
			spec->precision = spec->width - 2;
	}
}

int	ft_print_x(unsigned int arg, t_spec *spec)
{
	int	printed;

	printed = 0;
	config_x_spec(arg, spec);
	if (!spec->left_adjust)
	{
		if (spec->precision_flag && spec->precision == 0 && arg == 0)
			printed += ft_print_precision(spec->precision, spec->width, spec);
		else
			printed += ft_print_x_hex(arg, spec->precision, spec->width, spec);
	}
	else
	{
		if (spec->precision_flag && spec->precision == 0 && arg == 0)
			printed += ft_print_precision(spec->precision, 0, spec);
		else
			printed += ft_print_x_hex(arg, spec->precision, 0, spec);
		while (printed < spec->width)
			printed += write(1, " ", 1);
	}
	return (printed);
}
